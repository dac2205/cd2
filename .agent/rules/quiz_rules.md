...
- **Source**: `brand.json` (description) + `jtbd.json` -> `quiz/quiz_1.json`.
- **Constraint**: 
    - **Story-Driven**: Questions must reference specific brand details (e.g., ingredients, founders, technology) found in `brand.json` description array.
    - **Length**: concise (12-15 words).
    - **Quantity**: Exactly 20 questions per quiz file.
    - **Variety**: Generate only 1 quiz file per brand (`quiz_1.json`).

## 3. Storage
- **Location**: `data/brands/{brand_slug}/quiz/quiz_1.json`.
...
- **Randomization**:
    - **Questions**: Shuffle the 20 questions from `quiz_1.json`.
